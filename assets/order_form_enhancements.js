(() => {
  ...
  function updateSubmitState() {
    ...
    const hasRestrictions = Array.isArray(source.restrictions) && source.restrictions.length > 0;
    const acknowledged = !hasRestrictions || Boolean(document.querySelector('#corp-order-acknowledge')?.checked);
  ...
  function renderModal() {
    ...
    overlay.innerHTML = `
      <div class="corp-order-card" role="dialog" aria-modal="true" aria-labelledby="corp-order-title">
        <div class="corp-order-head"><div><h3 id="corp-order-title">Add to Corporate Order</h3><p class="corp-order-subtle">Office ${escapeHtml(payload.officeNumber)} - ${escapeHtml(payload.location)}</p></div><button type="button" class="corp-order-close" aria-label="Close">×</button></div>
        ...
        <div id="corp-order-warning" class="corp-order-warning hidden"></div><div id="corp-order-error" class="corp-order-error hidden"></div>
        <div class="corp-order-actions"><button type="button" class="corp-order-secondary" id="corp-order-cancel">Cancel</button><button type="button" class="corp-order-primary" id="corp-order-submit">Add to Order</button></div>
      </div>`;
    ...
    overlay.querySelector('#corp-order-source-select')?.addEventListener('change', (event) => { state.selectedSourceKey = event.target.value; updateRestrictionPanel(); });
  ...
  async function openOrderDialog(partId) {
    ...
    try {
      ...
      state.partId = Number(partId); state.payload = data;
      ...
      renderModal();
    } catch (error) { window.alert(error.message || 'Unable to load corporate ordering options.'); closeModal(); }
  }
  ...
  async function submitOrder() {
    ...
    try {
      const response = await fetch('/api/order-list-v2', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ warehouseId:currentWarehouseId(), partId:state.partId, quantity, sourceKey:source.sourceKey, acknowledgedRestrictions:acknowledged, notes }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Unable to add this item to the order list.');
      closeModal(); window.alert(`${data.partNumber} added to the ${data.vendor} order form.`); window.location.reload();
    } catch (error) {
      if (errorBox) { errorBox.textContent = error.message || 'Unable to add this item to the order list.'; errorBox.classList.remove('hidden'); }
      if (submit) { submit.textContent = 'Add to Order'; updateSubmitState(); }
    }
  }
  ...
  async function generateCorporateForms(button) {
    ...
    try {
      const response = await fetch('/api/order-forms/generate', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({warehouseId}) });
      if (!response.ok) {
        let message='Unable to generate corporate order forms.';
        try { const data=await response.json(); message=data.error||message; if(Array.isArray(data.unmappedParts)&&data.unmappedParts.length) message+=`\n\nRe-add these items with a corporate form selected: ${data.unmappedParts.join(', ')}`; } catch(_error) {}
        throw new Error(message);
      }
      const blob=await response.blob(); const disposition=response.headers.get('Content-Disposition')||''; const match=disposition.match(/filename="?([^";]+)"?/i); const filename=match?.[1]||'Office_93_Order_Forms.zip';
      const url=URL.createObjectURL(blob); const link=document.createElement('a'); link.href=url; link.download=filename; document.body.appendChild(link); link.click(); link.remove(); window.setTimeout(()=>URL.revokeObjectURL(url),1500);
    } catch(error) { window.alert(error.message || 'Unable to generate corporate order forms.'); }
    finally { if(button){button.disabled=false;button.textContent=originalText||'Generate Corporate Forms';} }
  }

  function relabelGenerateButton() { const button=document.querySelector('#order-list-generate'); if(button&&!button.dataset.corporateOrderRelabeled){button.textContent='Generate Corporate Forms';button.dataset.corporateOrderRelabeled='1';} }

  document.addEventListener('click',(event)=>{
    const orderButton=event.target.closest('[data-add-to-order-list], [data-inventory-scan-add-to-order]');
    if(orderButton){event.preventDefault();event.stopImmediatePropagation();const partId=orderButton.dataset.addToOrderList||orderButton.dataset.inventoryScanAddToOrder;openOrderDialog(partId);return;}
    const generateButton=event.target.closest('#order-list-generate');
    if(generateButton){event.preventDefault();event.stopImmediatePropagation();generateCorporateForms(generateButton);}
  },true);
  document.addEventListener('keydown',(event)=>{if(event.key==='Escape'&&state.open)closeModal();});
